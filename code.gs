...
function exportTrainingRecordToPdf(employee, records) {
  if (!employee || !records || records.length === 0) {
    throw new Error('Employee data and records are required for PDF export.');
  }

  const doc = DocumentApp.create(`Training Record - ${employee.employeeName}`);
  const body = doc.getBody();

  // --- Title and Employee Info ---
  const title = body.appendParagraph(`Individual Training Record for ${employee.employeeName}`);
  title.setHeading(DocumentApp.ParagraphHeading.HEADING1);
  title.setAlignment(DocumentApp.HorizontalAlignment.CENTER);
  body.appendParagraph(`Employee Number: ${employee.employeeNumber}`);
  body.appendParagraph(`Department: ${employee.department}`);
  body.appendParagraph(`Position: ${employee.position}`);
  body.appendParagraph(`FA/BU: ${employee.fabu}`);
  body.appendParagraph(`Location: ${employee.location}`);
  body.appendHorizontalRule();

  // --- Training History Table ---
  if (records.length > 0) {
    const historyTitle = body.appendParagraph('Training History');
    historyTitle.setHeading(DocumentApp.ParagraphHeading.HEADING2);
    const tableData = [['No.', 'Course Title', 'Start Date', 'End Date', 'Duration (Hrs)', 'Facilitator', 'Category', 'Remarks']];
    records.forEach(rec => {
      tableData.push([
        rec.no,
        rec.courseTitle,
        rec.trainingStarted,
        rec.trainingEnded,
        rec.trainingDuration.replace(' hours', ''),
        rec.facilitator,
        rec.trainingCategory,
        rec.remarks
      ]);
    });
  ...
  // --- Convert to PDF and return as Base64 ---
  ...
  return { 
    fileName: `ITR_${employee.employeeName.replace(/ /g, '_')}.pdf`, 
    mimeType: 'application/pdf', 
    data: pdfBase64 
  };
}

function exportTrainingRecordToSheet(employee, records) {
  if (!employee || !records || records.length === 0) {
    throw new Error('Employee data and records are required for Sheet export.');
  }

  const sheetName = `ITR_${employee.employeeName}`;
  const ss = SpreadsheetApp.create(sheetName);
  const sheet = ss.getActiveSheet();

  // --- Employee Info ---
  sheet.getRange('A1:B1').setValues([['Employee Name:', employee.employeeName]]).setFontWeight('bold');
  sheet.getRange('A2:B2').setValues([['Employee Number:', employee.employeeNumber]]);
  sheet.getRange('A3:B3').setValues([['Department:', employee.department]]);
  sheet.getRange('A4:B4').setValues([['Position:', employee.position]]);
  sheet.getRange('A5:B5').setValues([['FA/BU:', employee.fabu]]);
  sheet.getRange('A6:B6').setValues([['Location:', employee.location]]);
  
  // --- Training History Table ---
  const header = ['No.', 'Course Title', 'Start Date', 'End Date', 'Duration (Hrs)', 'Facilitator', 'Category', 'Remarks'];
  ...
  sheet.getRange(9, 1, tableData.length, tableData[0].length).setValues(tableData);
